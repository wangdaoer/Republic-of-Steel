# 《钢铁共和国》项目计划

状态：`CANON / PLAN`

## 阶段一：Canon资产沉淀

状态：`ACCEPTED`

- 世界观总纲
- 历史时间线
- 人物数据库
- 科技数据库
- 文明与势力数据库
- 术语百科

## 阶段二：GitHub与Codex工程初始化

状态：`ACCEPTED`

- 根目录工程结构
- `AGENTS.md` Codex规则
- Prompt模板入口
- 工具目录与资产目录
- 项目计划与开发日志
- Issue模板

## 阶段三：第一卷生产数据库

状态：`ACCEPTED / PRODUCTION_READY`

目标：建立《废铁营》的卷级大纲、120章目录、章节契约、人物状态、时间线和伏笔表。

计划目录：

```text
novel/volume01-waste-iron-camp/
├── outline.md
├── chapters/
├── characters.md
├── timeline.md
└── foreshadowing.md
```

## 阶段四：正文生产与全书闭合

状态：`COMPLETE / DRAFT`

四卷正文已经完成，所有章节均保留章节契约、正文、人物变化、世界变化、伏笔记录与审稿状态。

### 完成进度

| 卷 | 范围 | 状态 |
|---|---:|---|
| 第一卷《废铁营》 | 120章 | DRAFT / REVIEW_REQUIRED |
| 第二卷《守住北线》 | 40章 | DRAFT / REVIEW_REQUIRED |
| 第三卷《第一座工厂》 | 40章 | DRAFT / REVIEW_REQUIRED |
| 第四卷《星海拓荒》 | 40章 | COMPLETE / DRAFT，结构验收通过 |

总计：240章正文初稿。

全书时间线从SE3的404启动、SE5废铁营、SE350创造者协议推进至SE5000钢铁共和国原则；第四卷结构验收确认S-00、404、北轨、外部文明、不融合和终章交接保持Canon一致。

### 后续文学工作

- 四卷逐章语言精修
- 重复意象、章节节奏与长时间跳跃的回读校验
- 逐章审稿状态由REVIEW_REQUIRED推进至ACCEPTED
- 视觉资产、百科站和跨媒体资料整理

## 阶段五：发布与百科化

状态：`IN_PROGRESS / LITERARY_REVIEW`

计划包括版本发布、视觉资产整理、百科网站和跨媒体改编资料。

### 当前精修批次

- 第一卷开篇001—010已完成第一轮文学精修，均保留为 `DRAFT / POLISH_PASS_1 / FINAL_REVIEW_REQUIRED`
- 连续性债务 C-002、C-003 已修复：第005—006章的相对时间已与B-17救援和评估链对齐
- 下一批：第一卷011—020，完成第一篇“废铁里的火种”的语言精修与独立复审
